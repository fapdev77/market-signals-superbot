...
function normalizePricePrecision(value: number | null | undefined): number {
...
export function getOrderedModelChain(
  ...
  const fallbackModels = availableModels
    .filter(m => !m.isActive && m.isFallback)
    .sort((a, b) => a.priority - b.priority);

  // 1. If a specific active model was requested, put it first
  if (requestedModelIdentifier && requestedModelIdentifier !== 'none' && requestedModelIdentifier !== 'auto') {
    const requestedActive = activeModels.find(
      m => m.id === requestedModelIdentifier || m.modelId === requestedModelIdentifier || m.name === requestedModelIdentifier
    );
    if (requestedActive) {
      chain.push(requestedActive);
      addedIds.add(requestedActive.id);
    }
  }

  // 2. Add remaining active models sorted by priority
  ...
  // 3. Add fallback models sorted by priority
  ...
  // 4. If chain is empty (no active or fallback models), check if requested exists at all
  if (chain.length === 0 && requestedModelIdentifier && requestedModelIdentifier !== 'none') {
    const requested = availableModels.find(
      m => m.id === requestedModelIdentifier || m.modelId === requestedModelIdentifier || m.name === requestedModelIdentifier
    );
    if (requested) {
      chain.push(requested);
      addedIds.add(requested.id);
    }
  }

  // 5. Default fallback safety net
  if (chain.length === 0) {
    chain.push({
      id: 'default-gemini',
...
export function resolveTargetModel(
  ...
  const chain = getOrderedModelChain(requestedModelIdentifier, availableModels);
  return chain[0];
}
...
Risco/Retorno Mínimo Almejado: ${persona.minRRRatio}:1

Review the following live signal generated by Market Signals SuperBot:
Asset: ${ticker.symbol} (${ticker.name})
Current Price: ${formatPriceString(ticker.price)} (${ticker.price})
Signal Direction: ${signal.direction} (${signal.signalType})
Entry Zone: [${formatPriceString(signal.entryZone?.[0])} - ${formatPriceString(signal.entryZone?.[1])}]
Stop Loss: ${formatPriceString(signal.stopLoss)}
Target 1: ${formatPriceString(signal.target1)}
Target 2: ${formatPriceString(signal.target2)}
...
4. Provide your response as JSON.`;
  ...
  for (const targetModelConfig of modelChain) {
    try {
      ...
      let entryMin = normalizePricePrecision(json.entryMin) || normalizePricePrecision(signal.entryZone?.[0]) || normalizePricePrecision(ticker.price * 0.998);
      let entryMax = normalizePricePrecision(json.entryMax) || normalizePricePrecision(signal.entryZone?.[1]) || normalizePricePrecision(ticker.price);
      if (entryMin > entryMax) [entryMin, entryMax] = [entryMax, entryMin];

      let stopLoss = normalizePricePrecision(json.stopLoss) || normalizePricePrecision(signal.stopLoss);
      let takeProfit1 = normalizePricePrecision(json.takeProfit1) || normalizePricePrecision(signal.target1);
      let takeProfit2 = normalizePricePrecision(json.takeProfit2) || normalizePricePrecision(signal.target2);

      if (recommendedDirection === 'SHORT') {
        const midEntry = (entryMin + entryMax) / 2;
        const risk = Math.abs(stopLoss - midEntry) || (midEntry * 0.015);
        if (stopLoss <= entryMax) stopLoss = normalizePricePrecision(entryMax + risk);
        if (takeProfit1 >= entryMin) takeProfit1 = normalizePricePrecision(entryMin - risk * 1.5);
        if (takeProfit2 >= takeProfit1) takeProfit2 = normalizePricePrecision(takeProfit1 - risk * 1.5);
      } else if (recommendedDirection === 'LONG') {
        const midEntry = (entryMin + entryMax) / 2;
        const risk = Math.abs(midEntry - stopLoss) || (midEntry * 0.015);
        if (stopLoss >= entryMin) stopLoss = normalizePricePrecision(entryMin - risk);
        if (takeProfit1 <= entryMax) takeProfit1 = normalizePricePrecision(entryMax + risk * 1.5);
        if (takeProfit2 >= takeProfit1) takeProfit2 = normalizePricePrecision(takeProfit1 + risk * 1.5);
      }
      ...
      return {
        ...
        takeProfit1: normalizePricePrecision(takeProfit1),
        takeProfit2: normalizePricePrecision(takeProfit2),
...
function getFallbackSignalReview(ticker: TickerData, signal: TradeSignal, reason: string): AIReviewResponse {
  ...
  let entryMin = normalizePricePrecision(signal.entryZone?.[0]);
  let entryMax = normalizePricePrecision(signal.entryZone?.[1]);
  if (!entryMin || entryMin <= 0) entryMin = normalizePricePrecision(isLong ? price * 0.998 : price);
  if (!entryMax || entryMax <= 0) entryMax = normalizePricePrecision(isLong ? price : price * 1.002);
  if (entryMin > entryMax) [entryMin, entryMax] = [entryMax, entryMin];

  let stopLoss = normalizePricePrecision(signal.stopLoss);
  if (!stopLoss || stopLoss <= 0) {
    stopLoss = normalizePricePrecision(isLong ? price * 0.985 : price * 1.015);
  }

  let takeProfit1 = normalizePricePrecision(signal.target1);
  if (!takeProfit1 || takeProfit1 <= 0) {
    takeProfit1 = normalizePricePrecision(isLong ? price * 1.02 : price * 0.98);
  }

  let takeProfit2 = normalizePricePrecision(signal.target2);
  if (!takeProfit2 || takeProfit2 <= 0) {
    takeProfit2 = normalizePricePrecision(isLong ? price * 1.04 : price * 0.96);
  }
...
export async function chatWithAITrader(
  ...
  for (const targetModelConfig of modelChain) {
    try {
      const result = await generateContentWithModel(targetModelConfig, {
        prompt,
        systemInstruction: `Você é o SuperBot AI Trader, atuando com a seguinte persona: [${persona.name}]. ${persona.systemPromptAddendum} Responda de forma direta, técnica e prática em português.`,
        logType: 'CHAT_AGENT'
      });
  ...
  return {
    ...
    modelUsed: `Falha na IA (${lastErrorMsg})`
  };
}
